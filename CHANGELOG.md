...
- **`ScheduleType::Once`**: a one-shot / manual-only agent that the scheduler never auto-triggers — it runs via manual invoke/execute (or chat delegation), and after a successful run transitions to the new **`AgentStatus::Completed`** (previously an unrepresentable state: the frontend's "on-demand" option was an `interval_seconds: 0` encoding, rejected by create and never Completing). The editor's on-demand option now sends `once` (legacy `0` rows still read correctly), and the agent card renders Completed with its own badge. `invoke`/`execute` are the natural triggers for this form — the building block for chat-side delegation.
