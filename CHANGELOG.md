...
### OpenAPI schema — first batch live (auth + devices + telemetry)
- **`GET /api/docs/openapi.json`** now serves a real utoipa-generated OpenAPI 3.0 spec for the domains third parties integrate against first: auth (login/register/keys), device CRUD (incl. the upsert `updated_existing` note and `offline_timeout_secs` tri-state), and the full telemetry contract (every query parameter — `hours`, `aggregate` with its valid set, exclusive `cursor`, `history=true` — documented with units and semantics). The Scalar console at `/api/docs` now renders from this spec (parameters, response descriptions) instead of the bare route index; `routes.json` remains for the 329 not-yet-schematized routes. A CI drift test fails when an annotated path is not actually routed, so the spec cannot rot. Code generation (`openapi-generator` → typed clients) works against the spec today; remaining domains get annotated incrementally.
