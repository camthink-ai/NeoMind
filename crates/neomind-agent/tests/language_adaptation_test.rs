...
async fn test_chinese_input_chinese_response() -> Result<()> {
    ...
    assert!(
        is_chinese(&response),
        "Expected Chinese response but got: {}",
        response
    );
...
async fn test_english_input_english_response() -> Result<()> {
    ...
    assert!(
        is_english(&response),
        "Expected English response but got: {}",
        response
    );
...
async fn test_multilingual_conversation() -> Result<()> {
    ...
    assert!(
        is_chinese(&response2),
        "Expected Chinese response, got: {}",
        response2
    );
...
async fn test_mixed_language_handling() -> Result<()> {
    ...
    assert!(
        is_chinese(&response),
        "Expected Chinese response for mixed input, got: {}",
        response
    );
...
async fn test_all_languages_comprehensive() -> Result<()> {
    ...
    for (input, description, expect_chinese) in &test_cases {
        ...
        println!(
            ...
            if is_chinese_response {
                "Chinese"
            } else if is_english_response {
                "English"
            } else {
                "Mixed/Unknown"
            }
    ...
    assert!(
        pass_rate >= 0.66,
        "Pass rate {}% is below 66%",
        (pass_rate * 100.0) as i32
    );
