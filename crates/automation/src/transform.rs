...
//! - Code executes in a sandboxed QuickJS context
...
/// JavaScript-based transform executor using QuickJS
///
...
impl JsTransformExecutor {
    ...
    pub fn execute(
        ...
        let result_value = self.js_value_to_json(result)?;
    ...
    fn js_value_to_json(&self, value: quick_js::JsValue) -> Result<Value> {
        match value {
            quick_js::JsValue::Null => Ok(Value::Null),
            quick_js::JsValue::Undefined => Ok(Value::Null),
            quick_js::JsValue::Bool(b) => Ok(Value::Bool(b)),
            quick_js::JsValue::Int(i) => Ok(Value::Number(serde_json::Number::from(i))),
            quick_js::JsValue::Float(f) => {
            ...
            quick_js::JsValue::String(s) => Ok(Value::String(s)),
            quick_js::JsValue::Array(arr) => {
                let elements: Result<Vec<Value>> = arr.into_iter()
                    .map(|v| self.js_value_to_json(v))
                    .collect();
                Ok(Value::Array(elements?))
            }
            quick_js::JsValue::Object(obj) => {
                let mut map = serde_json::Map::new();
                for (key, val) in obj.into_iter() {
                    map.insert(key, self.js_value_to_json(val)?);
                }
                Ok(Value::Object(map))
            }
            // Catch-all for any other JsValue variants
            _ => Ok(Value::Null),
        }
    }
    ...
    fn json_to_metrics(
        ...
        match value {
            ...
            // String: use hash or try to parse as number
            Value::String(s) => {
