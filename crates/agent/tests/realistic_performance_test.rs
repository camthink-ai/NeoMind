    ...
    async fn llm_analyze(&self, system_prompt: &str, user_input: &str) -> (String, u128) {
        ...
        let output = self.llm_runtime.generate(input).await.unwrap_or_else(|e| {
            edge_ai_core::llm::backend::LlmOutput {
                text: format!("Error: {}", e),
                finish_reason: edge_ai_core::llm::backend::FinishReason::Error,
                usage: None,
                thinking: None,
            }
        });
    ...
    let parallel_futures: Vec<_> = queries.iter()
        .map(|query| {
            ...
            tokio::spawn(async move {
                let messages = vec![
                    edge_ai_core::message::Message::new(
                        edge_ai_core::message::MessageRole::System,
                        edge_ai_core::message::Content::text(&prompt)
                    ),
                    edge_ai_core::message::Message::new(
                        edge_ai_core::message::MessageRole::User,
                        edge_ai_core::message::Content::text(&q)
                    ),
                ];

                let input = edge_ai_core::llm::backend::LlmInput {
                    messages,
                    params: edge_ai_core::llm::backend::GenerationParams {
                        temperature: Some(0.7),
                        max_tokens: Some(500),
                        ..Default::default()
                    },
