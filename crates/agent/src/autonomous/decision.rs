...
use super::review::{ReviewResult, DetectedIssue, Recommendation, IssueSeverity};
...
impl DecisionEngine {
    ...
    pub async fn generate_decisions_from_review(
        ...
        match review_result {
            ReviewResult::Findings {
                ...
                if issues.len() > 1 {
                    if let Some(decision) = self.generate_summary_decision(issues, recommendations).await {
                        decisions.push(decision);
                    }
                }
    ...
    async fn generate_decision_from_recommendation(
        ...
        for suggested_action in &rec.actions {
            let action = DecisionAction::new(
                ...
                suggested_action.parameters.clone().unwrap_or(serde_json::json!({})),
            );
    ...
    async fn generate_summary_decision(
        ...
        let reasoning = format!(
            "Multiple issues detected across the system. Review individual issues for details."
        );
    ...
    pub async fn publish_decision(&self, decision: &Decision) {
        let event = NeoTalkEvent::LlmDecisionProposed {
            ...
            actions: decision.actions.iter().map(|a| {
                ProposedAction::new(
                    a.action_type.clone(),
                    a.description.clone(),
                    a.parameters.clone(),
        ...
        let _ = self.event_bus.publish_with_source(event, "decision_engine").await;
    }
    ...
    pub async fn try_execute_decision(&self, decision: &Decision) -> Result<(), DecisionError> {
        ...
        tracing::info!("Auto-executing decision: {} (confidence: {:.1}%)",
            decision.title, decision.confidence);
        ...
        let _ = self.event_bus.publish(NeoTalkEvent::LlmDecisionExecuted {
            decision_id: decision.id.clone(),
            success,
            result,
            timestamp: chrono::Utc::now().timestamp(),
        ...
        if success {
            ...
            Err(DecisionError::ActionFailed(format!(
                "{} of {} actions failed", failed_actions.len(), decision.actions.len()
            )))
        }
    }
    ...
    async fn execute_device_control_action(&self, action: &DecisionAction) -> Result<(), DecisionError> {
        ...
        let params = action.parameters.get("params")
    ...
    async fn execute_create_rule_action(&self, action: &DecisionAction) -> Result<(), DecisionError> {
        let dsl = action.parameters.get("dsl")
            .or_else(|| action.parameters.get("rule"))
            .and_then(|v| v.as_str())
            .ok_or_else(|| DecisionError::InvalidDecision("Missing DSL for rule creation".to_string()))?;
        ...
        // TODO: Integrate with RuleEngine to actually create the rule
        // This would require the DecisionEngine to hold a reference to the RuleEngine

        Ok(())
    }

    /// Execute a workflow trigger action.
    async fn execute_trigger_workflow_action(&self, action: &DecisionAction) -> Result<(), DecisionError> {
        let workflow_id = action.parameters.get("workflow_id")
            .and_then(|v| v.as_str())
        ...
        let _ = self.event_bus.publish(NeoTalkEvent::WorkflowTriggered {
    ...
    async fn execute_update_settings_action(&self, action: &DecisionAction) -> Result<(), DecisionError> {
        ...
        // TODO: Integrate with settings manager
        // For now, just log the proposed change
