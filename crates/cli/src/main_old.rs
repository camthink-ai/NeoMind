...
async fn run_plugin_cmd(cmd: PluginCommand) -> Result<()> {
    ...
    match cmd {
        ...
        PluginCommand::Create { name, plugin_type, output } => {
            create_plugin_via_script(&name, &plugin_type, output)?;
            Ok(())
        }
...
/// Create a new plugin scaffold using the shell script.
fn create_plugin_via_script(name: &str, plugin_type: &str, output: Option<PathBuf>) -> Result<()> {
    // Validate plugin name
    if !name.chars().all(|c| c.is_ascii_lowercase() || c.is_ascii_digit() || c == '-') {
        anyhow::bail!(
            "Invalid plugin name '{}'. Use only lowercase letters, numbers, and hyphens.",
            name
        );
    }

    let output_dir = output.unwrap_or_else(|| PathBuf::from("./plugins").join(name));
    ...
    let struct_name = display_name.replace(" ", "");
...
"#, name);
    ...
    let lib_rs = format!(r#"//! {} for NeoTalk
//!
//! Plugin type: {}
...
impl Plugin for {} {{
    fn metadata(&self) -> &PluginMetadata {{
        static METADATA: PluginMetadata = {{
            ...
            meta.description = "A {} plugin for NeoTalk".to_string();
            meta.types = vec!["{}".to_string()];
            meta
        }};
        &METADATA
    }}
...
#[no_mangle]
pub extern "C" fn neotalk_plugin_create() -> *mut {} {{
...
#[no_mangle]
pub static neotalk_plugin_descriptor: edge_ai_core::plugin::NativePluginDescriptor =
    edge_ai_core::plugin::NativePluginDescriptor {{
        api_version: edge_ai_core::plugin::NEOTALK_PLUGIN_API_VERSION,
        id: "{}\0".as_ptr(),
        id_len: "{}".len(),
        name: "{}\0".as_ptr(),
        name_len: "{}".len(),
        version: "0.1.0\0".as_ptr(),
        version_len: "0.1.0".len() - 1,
        description: concat!("A {} plugin for NeoTalk\0").as_ptr(),
        description_len: "A {} plugin for NeoTalk".len() + 1,
        required_version: ">=1.0.0\0".as_ptr(),
        required_version_len: ">=1.0.0".len() - 1,
        create: neotalk_plugin_create as *const (),
        destroy: neotalk_plugin_destroy as *const () as *const (),
    }};
...
Copy the compiled library to the NeoTalk plugins directory:

```bash
# macOS
cp target/release/lib{}.dylib /var/lib/neotalk/plugins/

# Linux
cp target/release/lib{}.so /var/lib/neotalk/plugins/

# Windows
copy target/release\{}.dll C:\neotalk\plugins\
```
...
- `hello` - Test command that returns a greeting
        ...
        for entry in entries.filter_map(|e| e.ok()) {
            ...
            if path.extension().map_or(false, |e| e == "wasm") {
                ...
                if let Some(ref filter_type) = ty {
                    // Try to read metadata to check type
...
async fn show_plugin_info(path: &PathBuf) -> Result<()> {
    ...
    // Determine plugin type by extension
