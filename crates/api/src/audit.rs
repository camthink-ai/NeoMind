...
impl AuditEntry {
    ...
    pub fn to_json(&self) -> String {
        serde_json::to_string(self).unwrap_or_else(|_| format!("{{\"action\": \"{}\"}}", self.action))
    }
...
impl AuditLogger {
    ...
    pub async fn log_auth_success(&self, user_id: &str, ip: &str) {
        self.log(AuditEntry::new(AuditSeverity::Info, AuditCategory::Authentication, "Authentication successful")
            .with_user(user_id)
            .with_ip(ip)).await;
    }
    ...
    pub async fn log_auth_failure(&self, ip: &str, reason: &str) {
        self.log(AuditEntry::new(AuditSeverity::Warning, AuditCategory::Authentication, format!("Authentication failed: {}", reason))
            .with_ip(ip)).await;
    }

    /// Log authorization success.
    pub async fn log_authz_success(&self, user_id: &str, resource: &str, action: &str) {
        self.log(AuditEntry::new(AuditSeverity::Info, AuditCategory::Authorization, format!("Authorized: {}", action))
            .with_user(user_id)
            .with_resource(resource)).await;
    }

    /// Log authorization denial.
    pub async fn log_authz_denied(&self, user_id: &str, resource: &str, action: &str) {
        self.log(AuditEntry::new(AuditSeverity::Warning, AuditCategory::Authorization, format!("Access denied: {}", action))
            .with_user(user_id)
            .with_resource(resource)).await;
    }
    ...
    pub async fn log_data_access(&self, user_id: &str, resource: &str, method: &str, path: &str) {
        self.log(AuditEntry::new(AuditSeverity::Info, AuditCategory::DataAccess, "Data accessed")
            .with_user(user_id)
    ...
    pub async fn log_data_modification(&self, user_id: &str, resource: &str, action: &str) {
        self.log(AuditEntry::new(AuditSeverity::Info, AuditCategory::DataModification, format!("Data modified: {}", action))
            .with_user(user_id)
            .with_resource(resource)).await;
    }
    ...
    pub async fn log_config_change(&self, user_id: &str, setting: &str) {
        self.log(AuditEntry::new(AuditSeverity::Warning, AuditCategory::Configuration, format!("Configuration changed: {}", setting))
            .with_user(user_id)).await;
    }

    /// Log API request.
    pub async fn log_api_request(&self, method: &str, path: &str, status: u16, ip: &str, user_id: Option<String>) {
        let mut entry = AuditEntry::new(
            if status >= 500 { AuditSeverity::Error } else { AuditSeverity::Info },
...
pub fn init_audit_logger(config: AuditConfig) {
    GLOBAL_AUDIT_LOGGER.set(Arc::new(AuditLogger::with_config(config)))
        .ok();
}
...
pub fn audit_middleware() -> impl Fn(axum::extract::Request, axum::middleware::Next) -> std::pin::Pin<Box<dyn std::future::Future<Output = axum::response::Response> + Send>> + Clone {
    move |mut req: axum::extract::Request, next: axum::middleware::Next| {
        Box::pin(async move {
            ...
            let user_id = req.extensions_mut().remove::<crate::auth::ValidatedApiKey>()
                .map(|k| k.0);
            ...
            // Log the request
            if let Some(logger) = audit_logger() {
                logger.log_api_request(&method, &path, status, &ip, user_id).await;
            }
...
mod tests {
    ...
    fn test_audit_entry_creation() {
        let entry = AuditEntry::new(AuditSeverity::Info, AuditCategory::Authentication, "Test action")
            .with_user("user123")
            .with_ip("127.0.0.1");
    ...
    async fn test_log_methods() {
        ...
        logger.log_authz_success("user123", "/api/devices", "read").await;
        logger.log_authz_denied("user123", "/api/admin", "write").await;
        logger.log_data_access("user123", "/api/devices", "GET", "/api/devices").await;
        logger.log_data_modification("user123", "/api/devices", "create").await;
        logger.log_config_change("admin", "llm.model").await;
        logger.log_api_request("GET", "/api/test", 200, "127.0.0.1", Some("user123".to_string())).await;
    }
}
