//! Cryptographic utilities for secret encryption (API keys, LLM keys, …).
//!
...
//!
//! Lives in neomind-core so every crate (api, storage, …) can share the same
//! `data/encryption_key` — previously it was api-only and lower layers had to
//! either duplicate it or store secrets in plaintext.
