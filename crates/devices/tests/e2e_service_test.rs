...
use edge_ai_devices::mdl_format::{CommandDefinition, MetricDefinition, ParameterDefinition};
use edge_ai_devices::{
...
impl DeviceAdapter for TestAdapter {
    ...
    fn subscribe(&self) -> Pin<Box<dyn Stream<Item = edge_ai_devices::DeviceEvent> + Send + '_>> {
    ...
    fn connection_status(&self) -> edge_ai_devices::adapter::ConnectionStatus {
        if self.is_running() {
            edge_ai_devices::adapter::ConnectionStatus::Connected
        } else {
            edge_ai_devices::adapter::ConnectionStatus::Disconnected
        }
    }
