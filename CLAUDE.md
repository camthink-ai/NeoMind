...
### Agent
- **ScheduleType**: `Event | Cron | Interval | Manual` (Manual = manual-only: never auto-scheduled, invoke runs it any number of times, idles as `AgentStatus::Completed`). **ExecutionMode**: `Focused` (bound resources, single-pass) | `Free` (LLM-driven multi-round tool calling, default schedule is `free` when no resources bound)
...
- **Notifications have three triggers, and `judgment` means silence.** `notify.on` is `failure` (default — silence is health), `always` (every verdict), or `judgment` (the machinery sends **nothing**; the task decides in its own prompt whether the operator needs to hear from it, and reaches them through `neomind message send`). A configured `notify` also **switches off the legacy keyword sniffing** — `is_alert_decision` only runs when `agent.notify.is_none()`. Without that gate both paths fired, so a doubly-notified operator was the normal case, not an edge one.
...
- **Structured (L0) agents refuse to run without input.** A structured agent's entire input is its bound sources, read over `time_range_minutes` (default 60, per-resource, editable as "Look back" in the editor). When none of them produced anything in that window it **errors instead of publishing** — the last published value stands, the record says why, and the breaker paces retries. Publishing the memory summary instead would launder a stale reading as a freshly computed one (a real incident: a camera offline for three hours, its agent confidently reporting "正常" every 15 minutes). The refusal names each source and when it last reported, read from the *unbounded* latest on purpose.
- **Published AI fields carry their provenance**: `ai:<agent>:<field>` points at the run that produced it via `metadata.execution_id` (`neomind_agent::EXECUTION_ID_KEY`, one constant for writer and reader) and carries the model's own confidence in `quality`. `DecisionProcess::confidence` / `ReasoningStep::confidence` are `Option<f32>` — absent means the model gave none, not a default; the API omits the key rather than sending `null`, because `null * 100` renders as "0%".
- **Chat reaches agents three ways**: the `query_conclusion` / `run_now` tools (both take the agent's name as the user said it — never an id), and `shell` → `neomind agent …` for everything else (create/configure/inspect). The base prompt's tool hierarchy, the `agent-management` skill and the tool descriptions all name them; adding a tool without all three leaves it unreachable in practice.
...
- **Clippy driver PATH trap**: `rust-toolchain.toml` pins 1.92.0, and rustup's proxy honors it — but if Homebrew's cargo is first on PATH (`which cargo` → `/opt/homebrew/bin/cargo`) it does not, and `clippy-driver` comes from Homebrew too (1.98's lints differ — false "failures" like `unneeded_wildcard_pattern`). Prepend the toolchain bin dir so **both** resolve to 1.92.0:
  ```bash
  PATH="$HOME/.rustup/toolchains/1.92.0-aarch64-apple-darwin/bin:$PATH" \
    cargo clippy --workspace --all-targets --locked -- -D warnings
  ```
  Drop the `+1.92.0` — with the toolchain bin dir first, `cargo` *is* the 1.92.0 binary, and `cargo +1.92.0` fails with `error: no such command: +1.92.0`. Sanity-check with `cargo clippy --version` (must print `0.1.92`). CI is unaffected: it runs plain `cargo clippy` in a checkout where the proxy is on PATH, so the pin applies.
