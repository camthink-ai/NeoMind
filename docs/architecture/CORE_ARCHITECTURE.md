# NeoTalk 核心架构分析与重构设计

> **文档状态**: 📋 设计阶段
>
> 本文档描述 NeoTalk 的目标架构设计和重构方案。各模块的实施数度参见 [TASKS.md](TASKS.md)。
>
> **状态标记**:
> - ✅ 已实现
> - 🚧 进行中
> - 📋 计划中
> - ⭐ 核心重点

---

## 一、项目核心理解

### 1.1 核心定位

**NeoTalk 是一个可多部署的边缘 AI 自主物联网平台**

```
核心理念: 设备抽象 + 事件驱动 + LLM 自主决策

┌─────────────────────────────────────────────────────────────────────────────┐
│                        NeoTalk: LLM 作为大脑的架构                          │
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │                        数据源插件层                                  │   │
│  │  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐            │   │
│  │  │ HASS     │  │ 内置MQTT │  │ 外部MQTT │  │ Modbus   │  ... 更多  │   │
│  │  │ Adapter  │  │ Broker   │  │ Broker   │  │ Adapter  │            │   │
│  │  └──────────┘  └──────────┘  └──────────┘  └──────────┘            │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                │                                            │
│                                ▼                                            │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │                        事件总线 (Event Bus)                          │   │
│  │              所有组件通过事件解耦和通信                                │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                    │               │               │                       │
│        ┌───────────┘               │               └───────────┐           │
│        │                           │                           │           │
│        ▼                           ▼                           ▼           │
│  ┌──────────┐              ┌──────────┐              ┌──────────┐         │
│  │  规则    │              │  工作流  │              │  告警    │         │
│  │  引擎    │              │  引擎    │              │  系统    │         │
│  └──────────┘              └──────────┘              └──────────┘         │
│        │                           │                           │           │
│        └───────────────────────────┼───────────────────────────┘           │
│                                    ▼                                       │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │                        LLM 大脑 (AI Brain)                           │   │
│  │  ┌─────────────────────────────────────────────────────────────┐    │   │
│  │  │  自主能力                                                    │    │   │
│  │  │  • 定期检索数据和分析                                        │    │   │
│  │  │  • 总结系统状态和趋势                                        │    │   │
│  │  │  • 主动提出决策建议                                          │    │   │
│  │  │  • 执行预防性措施                                            │    │   │
│  │  └─────────────────────────────────────────────────────────────┘    │   │
│  │  ┌─────────────────────────────────────────────────────────────┐    │   │
│  │  │  交互能力                                                    │    │   │
│  │  │  • 回答用户问题                                              │    │   │
│  │  │  • 执行用户命令                                              │    │   │
│  │  │  • 创建/管理自动化规则                                        │    │   │
│  │  │  • 解释系统行为                                              │    │   │
│  │  └─────────────────────────────────────────────────────────────┘    │   │
│  │  ┌─────────────────────────────────────────────────────────────┐    │   │
│  │  │  工具集 (完整工具链)                                         │    │   │
│  │  │  • list_devices - 列出所有设备                               │    │   │
│  │  │  • query_data - 查询时序数据                                 │    │   │
│  │  │  • control_device - 控制设备                                 │    │   │
│  │  │  • list_rules/create_rule - 管理规则                         │    │   │
│  │  │  • trigger_workflow - 触发工作流                             │    │   │
│  │  │  • analyze_trends - 分析数据趋势                             │    │   │
│  │  │  • propose_decisions - 提出决策建议                          │    │   │
│  │  │  • execute_decision - 执行决策                               │    │   │
│  │  └─────────────────────────────────────────────────────────────┘    │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                    │                                       │
│  ┌─────────────────────────────────┼─────────────────────────────────────┐ │
│  │                                  │                                     │ │
│  ▼                                  ▼                                     ▼ ▼
│ ┌──────────┐                    ┌──────────┐                          ┌──────┐
│ │  存储    │                    │  命令    │                          │ 用户 │
│ │          │                    │  下发层  │                          │      │
│ └──────────┘                    └──────────┘                          └──────┘
└─────────────────────────────────────────────────────────────────────────────┘
```

### 命令下发架构 ⭐

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                        命令下发 (Command Downlink)                          │
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │                        命令源 (Command Source)                       │   │
│  │  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐  │   │
│  │  │  用户       │  │  LLM        │  │  规则引擎   │  │  工作流     │  │   │
│  │  │  手动控制   │  │  工具调用   │  │  EXECUTE    │  │  步骤       │  │   │
│  │  └─────────────┘  └─────────────┘  └─────────────┘  └─────────────┘  │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                  │                                          │
│                                  ▼                                          │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │                        命令队列 (Priority Queue)                     │   │
│  │  ┌─────────────────────────────────────────────────────────────┐    │   │
│  │  │  Critical  →  High  →  Normal  →  Low                      │    │   │
│  │  └─────────────────────────────────────────────────────────────┘    │   │
│  │  • 持久化队列防止丢失                                              │   │
│  │  • 支持优先级调度                                                  │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                  │                                          │
│                                  ▼                                          │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │                        命令处理器 (Processor)                         │   │
│  │  ┌─────────────────────────────────────────────────────────────┐    │   │
│  │  │  • 从队列获取命令                                              │    │   │
│  │  │  • 调用下行适配器发送                                          │    │   │
│  │  │  • 更新命令状态                                                │    │   │
│  │  │  • 处理超时和重试                                              │    │   │
│  │  └─────────────────────────────────────────────────────────────┘    │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                  │                                          │
│                                  ▼                                          │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │                        下行适配器 (Downlink Adapter)                  │   │
│  │  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐             │   │
│  │  │  MQTT        │  │  Modbus      │  │  HTTP        │  ...        │   │
│  │  │  Downlink    │  │  Downlink    │  │  Downlink    │             │   │
│  │  └──────────────┘  └──────────────┘  └──────────────┘             │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                  │                                          │
│                                  ▼                                          │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │                        设备响应确认                                   │   │
│  │  ┌─────────────────────────────────────────────────────────────┐    │   │
│  │  │  • 订阅响应 topic                                              │    │   │
│  │  │  • 解析执行结果                                                │    │   │
│  │  │  • 更新命令状态                                                │    │   │
│  │  │  • 发布 CommandExecuted 事件                                   │    │   │
│  │  └─────────────────────────────────────────────────────────────┘    │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │                        命令状态存储                                   │   │
│  │  Pending → Queued → Sending → WaitingAck → Completed               │   │
│  │                              ↓                                       │   │
│  │                        Failed / Timeout / Cancelled                  │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────────────────────┘
```

### 命令相关事件 ⭐

```rust
// 新增到 NeoTalkEvent

/// 命令已创建
CommandCreated { command_id, device_id, command_name, source, timestamp }

/// 命令已发送
CommandSent { command_id, device_id, timestamp }

/// 命令超时
CommandTimeout { command_id, device_id, timestamp }

/// 命令执行完成 (原有 DeviceCommandResult 升级)
CommandExecuted {
    command_id, device_id, command_name, source, success, error, timestamp
}
```

### 1.2 核心设计原则

#### 原则 1: 多部署能力
系统可部署在多个位置：
- 家庭/办公室边缘节点
- 云端集中管理
- 工厂现场
- 移动设备

每个实例独立运行，通过事件总线协作。

#### 原则 2: 插件化数据源
所有外部数据源以插件形式接入：
- **HASS**: 只是众多数据源之一
- **内置 MQTT Broker**: 用于本地设备
- **外部 MQTT Broker**: 连接远程设备
- **未来**: HTTP API, OPC-UA, LoRaWAN...

每个插件实现相同的 `DeviceAdapter` 接口。

#### 原则 3: MDL 设备抽象
- MDL (Machine Description Language) 是设备描述标准
- 所有设备映射到统一的 MDL 格式
- 系统知道设备的能力（指标、命令、参数）

#### 原则 4: LLM 深度集成
LLM 不仅是聊天界面：
- **数据流集成**: LLM 可以直接访问实时设备数据
- **业务流集成**: LLM 可以创建/修改规则和工作流
- **自主决策**: LLM 可以定期分析数据并主动提出建议
- **用户代理**: 用户通过 LLM 与系统交互

#### 原则 5: 事件驱动解耦
所有组件通过事件总线通信：
- 设备 → 事件 → 规则/工作流
- 规则/工作流 → 事件 → 设备控制
- LLM ↔ 事件 ↔ 所有组件

### 1.3 核心组件详解

#### 数据源插件层
**作用**: 连接外部世界，统一数据入口

**插件接口**:
```rust
pub trait DeviceAdapter: Send + Sync {
    fn name(&self) -> &str;              // 插件名称
    async fn start(&self) -> Result<()>; // 启动插件
    async fn stop(&self) -> Result<()>;  // 停止插件
    fn subscribe(&self) -> Stream<DeviceEvent>; // 事件流
}
```

**已实现的插件**:
- `MqttAdapter`: 订阅 MQTT broker 的设备消息
- `HassAdapter`: 订阅 HASS discovery topic
- `ModbusAdapter`: 轮询 Modbus 设备

**未来插件**:
- `HttpAdapter`: 定期拉取 HTTP API
- `OpcuaAdapter`: OPC-UA 工业协议
- `LorawanAdapter`: LoRaWAN 物联网协议

#### 事件总线
**作用**: 系统的神经系统，连接所有组件

**核心事件**:
- `DeviceOnline/Offline`: 设备上下线
- `DeviceMetric`: 设备指标更新（核心！）
- `DeviceCommandResult`: 命令执行结果
- `RuleTriggered/Executed`: 规则触发/执行
- `WorkflowTriggered/Completed`: 工作流触发/完成
- `AlertCreated`: 告警创建
- `LlmDecision`: LLM 决策建议（新增！）
- `PeriodicReviewReady`: 定期审查触发（新增！）

#### 规则引擎
**作用**: 实时响应设备数据变化

**DSL**: `RULE "name" WHEN device.metric > threshold DO actions END`

**订阅**: `DeviceMetric` 事件
**发布**: `RuleTriggered`, `DeviceCommand` 事件

**关键**: 自动评估，无需轮询

#### 工作流引擎
**作用**: 编排复杂的多步骤自动化

**触发器**: Event（事件）、Cron（定时）、Manual（手动）、LlmDecision（LLM 决策）

**步骤**: DeviceQuery、Condition、ExecuteCommand、SendAlert、WasmExecute

#### LLM 大脑（核心！）
**作用**: 系统的智能决策中心

**被动模式**（已有）:
- 回答用户问题
- 执行用户命令
- 创建规则和工作流

**主动模式**（新增！）:
- 定期检索数据（periodic review）
- 分析趋势和异常
- 主动提出决策建议
- 执行预防性措施

**双向集成**:
- 订阅事件 → 获取实时数据
- 发布事件 → 执行控制操作

---

## 二、当前架构问题

### 2.1 架构概览 (当前)

```
当前架构问题分析:

┌──────────────────────────────────────────────────────────────┐
│                       API Server                             │
│  ┌────────────────────────────────────────────────────┐     │
│  │  ServerState (上帝对象)                               │     │
│  │  - session_manager: Arc<SessionManager>              │     │
│  │  - mqtt_device_manager: Arc<MqttDeviceManager>        │     │
│  │  - time_series_storage: Arc<TimeSeriesStorage>        │     │
│  │  - rule_engine: Arc<RuleEngine>                       │     │
│  │  - alert_manager: Arc<AlertManager>                   │     │
│  │  - workflow_engine: Arc<RwLock<Option<...>>>          │     │
│  │  - embedded_broker: Option<Arc<EmbeddedBroker>>       │     │
│  └────────────────────────────────────────────────────┘     │
└──────────────────────────────────────────────────────────────┘
        │ 直接调用                       │ 直接调用
        ↓                               ↓
┌───────────────┐              ┌───────────────┐
│  MqttDevice   │              │   RuleEngine  │
│   Manager     │──数据流向?──→│   (无数据流)  │
│  1417 行      │              │   (独立评估)  │
└───────────────┘              └───────────────┘
```

### 2.2 关键问题

#### 问题 1: 数据流断裂

**现状**:
- `MqttDeviceManager` 接收设备数据
- `RuleEngine` 有 `ValueProvider`，但两者没有自动连接
- 规则引擎需要手动调用 `evaluate_rules()`，不会自动被数据变化触发

**影响**: 规则不会自动响应设备数据变化

#### 问题 2: 事件系统不完整

**现状**:
```rust
// crates/api/src/server.rs:25-57
pub struct DeviceStatusUpdate {  // 仅有这一个事件类型
    pub update_type: String,
    pub device_id: String,
    pub status: Option<String>,
    pub last_seen: Option<i64>,
}
```

**缺失**:
- 统一的事件类型定义
- 事件发布订阅机制
- 事件持久化
- 事件回放

#### 问题 3: 规则引擎与设备脱节

**现状**:
```rust
// crates/rules/src/engine.rs:244-262
pub async fn evaluate_rules(&self) -> Vec<RuleId> {
    // 从 ValueProvider 获取值
    if let Some(value) = self.value_provider.get_value(&rule.condition.device_id, &rule.condition.metric) {
        if rule.should_trigger(value) {
            triggered.push(id.clone());
        }
    }
}
```

**问题**:
- 规则引擎不知道何时有新数据到达
- 需要外部定期调用 `evaluate_rules()`
- 没有与设备数据流集成

#### 问题 4: 工作流与设备/规则无集成

**现状**:
- 工作流可以定义 `DeviceQuery` 和 `ExecuteCommand` 步骤
- 但没有自动触发机制
- Event 触发器没有实现

#### 问题 5: LLM 工具调用与核心系统隔离

**现状**:
- LLM 可以调用工具 (list_devices, query_data, control_device)
- 但工具返回的是 Mock 数据
- 没有与真实系统连接

#### 问题 6: HASS 被过度关注

**现状**:
- 大量代码与 HASS 相关
- HASS 只是众多数据源之一
- 应该统一为"设备适配器"模式

#### 问题 7: LLM 未被用作"大脑"

**现状**:
- LLM 只是被动响应聊天
- 没有主动分析数据的能力
- 没有定期审查机制
- 没有自主决策能力

---

## 三、目标架构设计

### 3.1 LLM 中心架构

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                        NeoTalk: LLM 作为大脑                                │
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │                    数据源插件层 (可插拔)                              │   │
│  │  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐            │   │
│  │  │   HASS   │  │MQTT Broker│  │  Modbus  │  │   HTTP   │  ...      │   │
│  │  └──────────┘  └──────────┘  └──────────┘  └──────────┘            │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                │ 发布 DeviceMetric                          │
│                                ▼                                            │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │                        事件总线 (Event Bus)                          │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│         │                    │                    │                       │
│         ▼                    ▼                    ▼                       │
│  ┌────────────┐      ┌────────────┐      ┌────────────┐                 │
│  │  规则引擎  │      │  工作流    │      │  告警系统  │                 │
│  │            │      │  引擎      │      │            │                 │
│  └────────────┘      └────────────┘      └────────────┘                 │
│         │                    │                    │                       │
│         └────────────────────┼────────────────────┘                       │
│                              ▼                                            │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │                        LLM 大脑                                      │   │
│  │  ┌─────────────────────────────────────────────────────────────┐    │   │
│  │  │  被动交互层 (用户驱动)                                       │    │   │
│  │  │  - 处理用户聊天消息                                          │    │   │
│  │  │  - 执行用户命令                                              │    │   │
│  │  │  - 回答用户问题                                              │    │   │
│  │  └─────────────────────────────────────────────────────────────┘    │   │
│  │  ┌─────────────────────────────────────────────────────────────┐    │   │
│  │  │  主动分析层 (定时驱动)                                       │    │   │
│  │  │  - PeriodicReviewTrigger (每N小时/天)                       │    │   │
│  │  │  - 检索设备数据                                              │    │   │
│  │  │  - 分析趋势和异常                                            │    │   │
│  │  │  - 生成决策建议                                              │    │   │
│  │  └─────────────────────────────────────────────────────────────┘    │   │
│  │  ┌─────────────────────────────────────────────────────────────┐    │   │
│  │  │  工具集 (连接真实系统)                                       │    │   │
│  │  │  - 设备: list_devices, query_data, control_device           │    │   │
│  │  │  - 规则: list_rules, create_rule, enable/disable_rule       │    │   │
│  │  │  - 工作流: list_workflows, trigger_workflow                  │    │   │
│  │  │  - 分析: analyze_trends, detect_anomalies                    │    │   │
│  │  │  - 决策: propose_decision, execute_decision                  │    │   │
│  │  └─────────────────────────────────────────────────────────────┘    │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│           │                             │                                 │
│           ▼                             ▼                                 │
│    ┌──────────┐                   ┌──────────┐                           │
│    │  存储    │                   │  用户    │                           │
│    │          │                   │  通知    │                           │
│    └──────────┘                   └──────────┘                           │
└─────────────────────────────────────────────────────────────────────────────┘
```

### 3.2 统一事件类型

```rust
// crates/core/src/event.rs

/// 统一的事件类型
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(tag = "type")]
pub enum NeoTalkEvent {
    // ========== 设备事件 ==========
    /// 设备上线
    DeviceOnline {
        device_id: String,
        device_type: String,
        timestamp: i64,
    },

    /// 设备离线
    DeviceOffline {
        device_id: String,
        reason: Option<String>,
        timestamp: i64,
    },

    /// 设备指标更新 (核心事件!)
    DeviceMetric {
        device_id: String,
        metric: String,
        value: MetricValue,
        timestamp: i64,
        quality: Option<f32>,
    },

    /// 设备命令完成
    DeviceCommandResult {
        device_id: String,
        command: String,
        success: bool,
        result: Option<serde_json::Value>,
        timestamp: i64,
    },

    // ========== 规则事件 ==========
    /// 规则条件评估
    RuleEvaluated {
        rule_id: String,
        rule_name: String,
        condition_met: bool,
        timestamp: i64,
    },

    /// 规则触发
    RuleTriggered {
        rule_id: String,
        rule_name: String,
        trigger_value: f64,
        actions: Vec<String>,
        timestamp: i64,
    },

    /// 规则执行完成
    RuleExecuted {
        rule_id: String,
        rule_name: String,
        success: bool,
        duration_ms: u64,
        timestamp: i64,
    },

    // ========== 工作流事件 ==========
    /// 工作流触发
    WorkflowTriggered {
        workflow_id: String,
        trigger_type: String,  // "event", "cron", "manual", "llm_decision"
        trigger_data: Option<serde_json::Value>,
        execution_id: String,
        timestamp: i64,
    },

    /// 工作流步骤完成
    WorkflowStepCompleted {
        workflow_id: String,
        execution_id: String,
        step_id: String,
        result: serde_json::Value,
        timestamp: i64,
    },

    /// 工作流完成
    WorkflowCompleted {
        workflow_id: String,
        execution_id: String,
        success: bool,
        duration_ms: u64,
        timestamp: i64,
    },

    // ========== 告警事件 ==========
    /// 告警创建
    AlertCreated {
        alert_id: String,
        title: String,
        severity: String,
        message: String,
        timestamp: i64,
    },

    /// 告警确认
    AlertAcknowledged {
        alert_id: String,
        acknowledged_by: String,
        timestamp: i64,
    },

    // ========== LLM 事件 (新增!) ==========
    /// 定期审查触发
    PeriodicReviewTriggered {
        review_id: String,
        review_type: String,  // "hourly", "daily", "weekly"
        timestamp: i64,
    },

    /// LLM 决策建议
    LlmDecisionProposed {
        decision_id: String,
        title: String,
        description: String,
        reasoning: String,
        actions: Vec<ProposedAction>,
        confidence: f32,
        timestamp: i64,
    },

    /// LLM 决策执行
    LlmDecisionExecuted {
        decision_id: String,
        success: bool,
        result: Option<serde_json::Value>,
        timestamp: i64,
    },

    // ========== 用户事件 ==========
    /// 用户消息 (LLM)
    UserMessage {
        session_id: String,
        content: String,
        timestamp: i64,
    },

    /// LLM 响应事件
    LlmResponse {
        session_id: String,
        content: String,
        tools_used: Vec<String>,
        processing_time_ms: u64,
        timestamp: i64,
    },
}

/// 提议的决策动作
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ProposedAction {
    pub action_type: String,  // "control_device", "create_rule", "notify_user"
    pub description: String,
    pub parameters: serde_json::Value,
}

/// 事件元数据
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct EventMetadata {
    pub event_id: String,
    pub correlation_id: Option<String>,
    pub causation_id: Option<String>,  // 因果关系链
    pub source: String,                // 事件源
    pub timestamp: i64,
}
```

### 3.3 事件总线实现

```rust
// crates/core/src/eventbus.rs

use tokio::sync::broadcast;
use std::sync::Arc;
use crate::event::NeoTalkEvent;

/// 事件总线
pub struct EventBus {
    /// 广播通道
    tx: broadcast::Sender<(NeoTalkEvent, EventMetadata)>,
}

impl EventBus {
    pub fn new(capacity: usize) -> Self {
        let (tx, _) = broadcast::channel(capacity);
        Self { tx }
    }

    /// 发布事件
    pub async fn publish(&self, event: NeoTalkEvent) {
        let metadata = EventMetadata {
            event_id: uuid::Uuid::new_v4().to_string(),
            correlation_id: None,
            causation_id: None,
            source: "system".to_string(),
            timestamp: chrono::Utc::now().timestamp(),
        };

        let _ = self.tx.send((event, metadata));
    }

    /// 订阅所有事件
    pub fn subscribe(&self) -> broadcast::Receiver<(NeoTalkEvent, EventMetadata)> {
        self.tx.subscribe()
    }

    /// 选择性订阅 (通过过滤器)
    pub async fn subscribe_filtered<F>(&self, filter: F) -> Subscription
    where
        F: Fn(&NeoTalkEvent) -> bool + Send + 'static,
    {
        let rx = self.subscribe();
        Subscription::new(rx, filter)
    }
}

/// 选择性订阅
pub struct Subscription {
    rx: broadcast::Receiver<(NeoTalkEvent, EventMetadata)>,
    filter: Box<dyn Fn(&NeoTalkEvent) -> bool + Send>,
}

impl Subscription {
    fn new<F>(rx: broadcast::Receiver<(NeoTalkEvent, EventMetadata)>, filter: F) -> Self
    where
        F: Fn(&NeoTalkEvent) -> bool + Send + 'static,
    {
        Self {
            rx,
            filter: Box::new(filter),
        }
    }

    pub async fn recv(&mut self) -> Option<(NeoTalkEvent, EventMetadata)> {
        loop {
            match self.rx.recv().await {
                Ok((event, meta)) => {
                    if (self.filter)(&event) {
                        return Some((event, meta));
                    }
                }
                Err(_) => return None,
            }
        }
    }
}
```

### 3.4 数据源适配器模式

```rust
// crates/devices/src/adapter.rs

/// 设备数据源适配器
#[async_trait]
pub trait DeviceAdapter: Send + Sync {
    /// 适配器名称
    fn name(&self) -> &str;

    /// 启动适配器
    async fn start(&self) -> Result<(), DeviceError>;

    /// 停止适配器
    async fn stop(&self) -> Result<(), DeviceError>;

    /// 订阅设备数据，返回事件流
    fn subscribe(&self) -> Pin<Box<dyn Stream<Item = DeviceEvent> + Send>>;
}

/// 设备事件
#[derive(Debug, Clone)]
pub enum DeviceEvent {
    Metric {
        device_id: String,
        metric: String,
        value: MetricValue,
        timestamp: i64,
    },
    State {
        device_id: String,
        old_state: DeviceState,
        new_state: DeviceState,
    },
    Discovery {
        device: DiscoveredDevice,
    },
}

/// MQTT 设备适配器
pub struct MqttAdapter {
    config: MqttConfig,
    event_tx: broadcast::Sender<NeoTalkEvent>,
}

/// HASS 设备适配器 (HASS 只是其中一个适配器)
pub struct HassAdapter {
    broker: String,
    port: u16,
    event_tx: broadcast::Sender<NeoTalkEvent>,
}

/// Modbus 设备适配器
pub struct ModbusAdapter {
    devices: Vec<ModbusDeviceConfig>,
    event_tx: broadcast::Sender<NeoTalkEvent>,
}

/// HTTP API 设备适配器
pub struct HttpAdapter {
    endpoints: Vec<HttpEndpointConfig>,
    event_tx: broadcast::Sender<NeoTalkEvent>,
}
```

### 3.5 规则引擎集成

```rust
// crates/rules/src/integration.rs

/// 事件驱动的规则引擎
pub struct EventDrivenRuleEngine {
    inner: RuleEngine,
    event_rx: broadcast::Receiver<(NeoTalkEvent, EventMetadata)>,
    event_tx: broadcast::Sender<NeoTalkEvent>,
}

impl EventDrivenRuleEngine {
    pub fn new(
        value_provider: Arc<dyn ValueProvider>,
        event_bus: &EventBus,
    ) -> Self {
        let rx = event_bus.subscribe();
        let tx = event_bus.sender();

        let engine = Self {
            inner: RuleEngine::new(value_provider),
            event_rx: rx,
            event_tx: tx,
        };

        // 启动事件处理循环
        tokio::spawn(engine.run());

        engine
    }

    /// 事件处理循环
    async fn run(mut self) {
        loop {
            match self.event_rx.recv().await {
                Ok((event, metadata)) => {
                    self.handle_event(event).await;
                }
                Err(broadcast::error::RecvError::Lagged(count)) => {
                    tracing::warn!("Rule engine lagged behind by {} events", count);
                }
                Err(broadcast::error::RecvError::Closed) => {
                    tracing::error!("Event bus closed, rule engine stopping");
                    break;
                }
            }
        }
    }

    /// 处理单个事件
    async fn handle_event(&self, event: NeoTalkEvent) {
        match event {
            // 设备指标更新 → 评估相关规则
            NeoTalkEvent::DeviceMetric { device_id, metric, value, timestamp } => {
                self.evaluate_metric_rules(&device_id, &metric, &value, timestamp).await;
            }

            // 设备状态变化 → 评估相关规则
            NeoTalkEvent::DeviceOnline { device_id, .. } => {
                self.evaluate_device_rules(&device_id).await;
            }

            _ => {}
        }
    }

    /// 评估与该设备指标相关的规则
    async fn evaluate_metric_rules(
        &self,
        device_id: &str,
        metric: &str,
        value: &MetricValue,
        timestamp: i64,
    ) {
        let rules = self.inner.list_rules().await;

        for rule in rules {
            // 检查规则是否匹配这个设备的这个指标
            if rule.condition.device_id == device_id && rule.condition.metric == metric {
                let numeric_value = match value {
                    MetricValue::Float(v) => *v,
                    MetricValue::Integer(v) => *v as f64,
                    _ => continue,
                };

                // 评估规则
                if rule.should_trigger(numeric_value) {
                    // 触发规则
                    self.trigger_rule(&rule.id, numeric_value, timestamp).await;
                }
            }
        }
    }

    /// 触发规则
    async fn trigger_rule(&self, rule_id: &RuleId, value: f64, timestamp: i64) {
        let result = self.inner.execute_rule(rule_id).await;

        // 发布规则触发事件
        let _ = self.event_tx.send(NeoTalkEvent::RuleTriggered {
            rule_id: rule_id.0.to_string(),
            rule_name: result.rule_name.clone(),
            trigger_value: value,
            actions: result.actions_executed,
            timestamp,
        });
    }
}
```

### 3.6 LLM 工具集成

```rust
// crates/agent/src/tools_integration.rs

/// 真实的工具注册表 (连接真实系统)
pub struct RealToolRegistry {
    device_manager: Arc<MqttDeviceManager>,
    rule_engine: Arc<RuleEngine>,
    telemetry_store: Arc<TimeSeriesStorage>,
    workflow_engine: Arc<WorkflowEngine>,
}

impl RealToolRegistry {
    pub fn new(
        device_manager: Arc<MqttDeviceManager>,
        rule_engine: Arc<RuleEngine>,
        telemetry_store: Arc<TimeSeriesStorage>,
        workflow_engine: Arc<WorkflowEngine>,
    ) -> Self {
        Self {
            device_manager,
            rule_engine,
            telemetry_store,
            workflow_engine,
        }
    }

    /// 执行工具
    pub async fn execute_tool(&self, name: &str, args: serde_json::Value) -> Result<ToolOutput> {
        match name {
            "list_devices" => {
                let devices = self.device_manager.list_devices().await;
                Ok(ToolOutput::success(serde_json::json!({ "devices": devices })))
            }

            "query_data" => {
                let device_id = args["device_id"].as_str().ok_or("missing device_id")?;
                let metric = args["metric"].as_str().ok_or("missing metric")?;
                let value = self.device_manager.read_metric(device_id, metric).await?;
                Ok(ToolOutput::success(serde_json::json!({ "value": value_to_json(&value) })))
            }

            "control_device" => {
                let device_id = args["device_id"].as_str().ok_or("missing device_id")?;
                let command = args["command"].as_str().ok_or("missing command")?;
                let result = self.device_manager.send_command(device_id, command, args).await?;
                Ok(ToolOutput::success(serde_json::json!({ "result": result })))
            }

            "list_rules" => {
                let rules = self.rule_engine.list_rules().await;
                Ok(ToolOutput::success(serde_json::json!({ "rules": rules })))
            }

            "create_rule" => {
                let dsl = args["dsl"].as_str().ok_or("missing dsl")?;
                let rule_id = self.rule_engine.add_rule_from_dsl(dsl).await?;
                Ok(ToolOutput::success(serde_json::json!({ "rule_id": rule_id.to_string() })))
            }

            _ => Err(ToolError::NotFound(name.to_string())),
        }
    }
}
```

### 3.7 LLM 主动决策能力 (新增！)

```rust
// crates/agent/src/autonomous.rs

/// LLM 自主决策系统
pub struct AutonomousAgent {
    agent: Arc<Agent>,
    event_bus: Arc<EventBus>,
    telemetry_store: Arc<TimeSeriesStorage>,
    config: AutonomousConfig,
}

#[derive(Debug, Clone)]
pub struct AutonomousConfig {
    /// 定期审查间隔 (秒)
    pub review_interval_secs: u64,

    /// 审查类型
    pub review_types: Vec<ReviewType>,
}

#[derive(Debug, Clone)]
pub enum ReviewType {
    /// 设备状态审查
    DeviceHealth,

    /// 趋势分析
    TrendAnalysis,

    /// 异常检测
    AnomalyDetection,

    /// 能耗优化
    EnergyOptimization,
}

impl AutonomousAgent {
    pub fn new(
        agent: Arc<Agent>,
        event_bus: Arc<EventBus>,
        telemetry_store: Arc<TimeSeriesStorage>,
        config: AutonomousConfig,
    ) -> Self {
        Self {
            agent,
            event_bus,
            telemetry_store,
            config,
        }
    }

    /// 启动自主决策循环
    pub fn start(self: Arc<Self>) {
        tokio::spawn(async move {
            let mut interval = tokio::time::interval(
                tokio::time::Duration::from_secs(self.config.review_interval_secs)
            );

            loop {
                interval.tick().await;
                self.run_review_cycle().await;
            }
        });
    }

    /// 运行一次审查周期
    async fn run_review_cycle(&self) {
        tracing::info!("Starting autonomous review cycle");

        // 发布审查触发事件
        let review_id = uuid::Uuid::new_v4().to_string();
        self.event_bus.publish(NeoTalkEvent::PeriodicReviewTriggered {
            review_id: review_id.clone(),
            review_type: "full".to_string(),
            timestamp: chrono::Utc::now().timestamp(),
        }).await;

        // 收集数据
        let context = self.collect_context().await;

        // 让 LLM 分析并提供建议
        let prompt = format!(
            "你是系统的智能大脑。请分析当前系统状态并提出决策建议。\n\n\
            当前状态:\n\
            - 在线设备: {}\n\
            - 离线设备: {}\n\
            - 活跃规则: {}\n\
            - 今日告警: {}\n\n\
            请分析:\n\
            1. 是否有异常情况?\n\
            2. 是否有优化空间?\n\
            3. 是否需要创建新的自动化规则?\n\n\
            以JSON格式返回你的决策建议。",
            context.online_devices,
            context.offline_devices,
            context.active_rules,
            context.today_alerts
        );

        // 调用 LLM
        match self.agent.think(&prompt).await {
            Ok(decisions) => {
                // 处理决策建议
                for decision in decisions {
                    self.handle_decision(decision).await;
                }
            }
            Err(e) => {
                tracing::error!("Autonomous review failed: {}", e);
            }
        }
    }

    /// 收集系统上下文
    async fn collect_context(&self) -> SystemContext {
        // 查询设备状态、规则状态、告警等
        // ...
    }

    /// 处理决策建议
    async fn handle_decision(&self, decision: Decision) {
        // 1. 发布 LlmDecisionProposed 事件
        self.event_bus.publish(NeoTalkEvent::LlmDecisionProposed {
            decision_id: decision.id.clone(),
            title: decision.title.clone(),
            description: decision.description.clone(),
            reasoning: decision.reasoning.clone(),
            actions: decision.actions.clone(),
            confidence: decision.confidence,
            timestamp: chrono::Utc::now().timestamp(),
        }).await;

        // 2. 如果置信度高且不需要用户确认，自动执行
        if decision.confidence > 0.8 && decision.auto_execute {
            for action in decision.actions {
                self.execute_action(&action).await;
            }
        }
    }

    /// 执行决策动作
    async fn execute_action(&self, action: &ProposedAction) {
        match action.action_type.as_str() {
            "control_device" => {
                // 控制设备
            }
            "create_rule" => {
                // 创建规则
            }
            "notify_user" => {
                // 通知用户
            }
            _ => {}
        }
    }
}
```

### 3.8 MDL/DSL 作为 LLM 知识库 (新增！)

**核心理念**: MDL 和 DSL 不仅是配置格式，更是 LLM 理解设备和规则的核心知识库。

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    LLM 知识库架构                                            │
│                                                                             │
│   ┌─────────────────┐         ┌─────────────────┐                           │
│   │   MDL 知识库    │         │   DSL 知识库    │                           │
│   │  (设备理解)     │         │  (规则理解)     │                           │
│   ├─────────────────┤         ├─────────────────┤                           │
│   │ • 设备类型      │         │ • 规则定义      │                           │
│   │ • 能力描述      │         │ • 触发条件      │                           │
│   │ • 指标定义      │         │ • 执行动作      │                           │
│   │ • 命令接口      │         │ • 约束条件      │                           │
│   └────────┬────────┘         └────────┬────────┘                           │
│            │                           │                                     │
│            └───────────┬───────────────┘                                     │
│                        ▼                                                   │
│           ┌─────────────────────────┐                                       │
│           │   双向翻译层            │                                       │
│           │ NL ⇄ MDL, NL ⇄ DSL     │                                       │
│           └───────────┬─────────────┘                                       │
│                       │                                                    │
│                       ▼                                                    │
│           ┌─────────────────────────┐                                       │
│           │      LLM Tools          │                                       │
│           │  • list_device_types    │                                       │
│           │  • get_device_type      │                                       │
│           │  • explain_device       │                                       │
│           │  • list_rules           │                                       │
│           │  • get_rule             │                                       │
│           │  • explain_rule         │                                       │
│           │  • generate_rule_dsl    │                                       │
│           └─────────────────────────┘                                       │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

#### MDL 为 LLM 提供设备理解

```rust
/// LLM 查询设备类型的工具
impl Tool for ListDeviceTypes {
    fn name(&self) -> &str { "list_device_types" }
    fn description(&self) -> &str { "查询系统中所有可用的设备类型" }

    async fn execute(&self, args: Value) -> ToolResult {
        let types = MdlRegistry::global().list_types();
        Ok(json!({ "device_types": types }))
    }
}

impl Tool for GetDeviceType {
    fn name(&self) -> &str { "get_device_type" }
    fn description(&self) -> &str { "获取指定设备类型的完整定义" }

    async fn execute(&self, args: Value) -> ToolResult {
        let type_id = args["type_id"].as_str()?;
        let definition = MdlRegistry::global().get_definition(type_id)?;
        Ok(json!(definition))  // 包含 uplink 指标、downlink 命令
    }
}

impl Tool for ExplainDeviceType {
    fn name(&self) -> &str { "explain_device_type" }
    fn description(&self) -> &str { "用自然语言解释设备类型功能" }

    async fn execute(&self, args: Value) -> ToolResult {
        let def = get_device_type(&args["type_id"])?;
        // 将 MDL 转换为自然语言描述
        let explanation = mdl_to_natural_language(&def, "zh");
        Ok(json!({ "explanation": explanation }))
    }
}
```

#### DSL 为 LLM 提供规则理解

```rust
/// LLM 查询规则的工具
impl Tool for ListRules {
    fn name(&self) -> &str { "list_rules" }
    fn description(&self) -> &str { "列出系统中的所有规则" }

    async fn execute(&self, args: Value) -> ToolResult {
        let rules = RuleEngine::global().list_rules();
        Ok(json!({ "rules": rules }))
    }
}

impl Tool for ExplainRule {
    fn name(&self) -> &str { "explain_rule" }
    fn description(&self) -> &str { "用自然语言解释规则的含义" }

    async fn execute(&self, args: Value) -> ToolResult {
        let rule = RuleEngine::global().get_rule(&args["rule_id"])?;
        // 将 DSL 转换为自然语言解释
        let explanation = dsl_to_natural_language(&rule, "zh");
        Ok(json!({ "explanation": explanation }))
    }
}

impl Tool for GenerateRuleDsl {
    fn name(&self) -> &str { "generate_rule_dsl" }
    fn description(&self) -> &str { "根据用户描述生成规则 DSL" }

    async fn execute(&self, args: Value) -> ToolResult {
        let description = args["description"].as_str()?;
        // 获取相关设备类型作为上下文
        let device_context = extract_device_context(description);
        // 调用 LLM 生成 DSL
        let dsl = self.llm.generate_with_context(description, device_context).await?;
        // 验证 DSL
        let validated = RuleDslParser::new().parse(&dsl)?;
        Ok(json!({ "dsl": dsl, "rule": validated }))
    }
}
```

#### 智能上下文注入

```rust
/// 根据用户查询选择相关上下文
pub struct ContextSelector {
    mdl_store: Arc<MdlStore>,
    dsl_store: Arc<DslStore>,
    vector_store: Arc<VectorStore>,
}

impl ContextSelector {
    pub async fn select_context(&self, query: &str) -> ContextBundle {
        let intent = IntentAnalyzer::analyze(query);

        match intent {
            Intent::DeviceQuery => {
                // 语义搜索相关设备类型
                bundle.mdl_definitions = self.vector_store.search(query, top_k=5).await;
            }
            Intent::RuleCreation => {
                // 创建规则需要所有设备类型和示例规则
                bundle.mdl_definitions = self.get_all_device_types().await;
                bundle.dsl_examples = true;
            }
            // ...
        }
        bundle
    }
}
```

#### 双向翻译

- **MDL → 自然语言**: 向用户解释设备能力
- **DSL → 自然语言**: 向用户解释规则逻辑
- **自然语言 → DSL**: 用户意图转换为规则
- **自然语言 → MDL 查询**: 用户问题转换为设备查询

#### 向量化存储

```rust
/// 索引 MDL 定义用于语义搜索
pub async fn index_mdl(&self, definition: &DeviceTypeDefinition) {
    let text = format!(
        "{} {} {} {}",
        definition.name,
        definition.description,
        definition.uplink.iter().map(|m| &m.name).join(" "),
        definition.downlink.iter().map(|c| &c.name).join(" ")
    );

    let embedding = self.embed_text(&text).await;

    self.vector_store.insert(VectorDocument {
        id: definition.type_id.clone(),
        vector: embedding,
        metadata: json!({ "type": "mdl", "name": definition.name }),
        payload: json!(definition),
    }).await;
}
```

---

## 四、重构计划

### 阶段 0: 基础设施 (Week 1)

**目标**: 建立事件驱动基础设施

1. **创建事件模块**
   - `crates/core/src/event.rs` - 统一事件类型定义（包括 LLM 事件）
   - `crates/core/src/eventbus.rs` - 事件总线实现
   - `crates/core/src/metadata.rs` - 事件元数据

2. **创建适配器接口**
   - `crates/devices/src/adapter.rs` - DeviceAdapter trait
   - `crates/devices/src/adapters/mod.rs` - 适配器模块

### 阶段 1: 数据源插件化 (Week 2)

**目标**: 将现有数据源重构为插件

1. **MQTT 适配器**
   - `crates/devices/src/adapters/mqtt.rs` - MQTT 适配器

2. **HASS 适配器**
   - `crates/devices/src/adapters/hass.rs` - HASS 适配器（只是其中一个）

3. **适配器管理器**
   - `crates/devices/src/adapter_manager.rs` - 管理所有适配器

### 阶段 2: 规则引擎事件驱动 (Week 3)

**目标**: 规则引擎自动响应设备事件

1. **重构规则引擎**
   - 创建 `EventDrivenRuleEngine`
   - 集成事件订阅
   - 自动评估规则

2. **数据流集成**
   - 设备指标事件 → 规则评估
   - 规则触发 → 设备控制事件

### 阶段 3: 工作流事件集成 (Week 4)

**目标**: 工作流响应事件触发

1. **工作流触发器**
   - 实现 EventTrigger
   - 订阅特定事件模式
   - 自动启动工作流

2. **设备集成**
   - DeviceQuery 步骤从真实设备读取
   - ExecuteCommand 步骤发送真实命令

### 阶段 4: LLM 工具集成 (Week 5)

**目标**: LLM 可以调用真实系统

1. **真实工具实现**
   - 替换 Mock 为真实系统调用
   - 连接 DeviceManager, RuleEngine, WorkflowEngine

2. **双向集成**
   - LLM 可以触发规则和工作流
   - 规则和工作流可以发送通知给用户

### 阶段 5: LLM 自主决策 (Week 6-7) ⭐ 核心阶段

**目标**: LLM 成为系统大脑

1. **定期审查机制**
   - `crates/agent/src/autonomous.rs` - 自主 Agent
   - 定时触发审查（每小时/每天）
   - 收集系统状态数据

2. **数据分析能力**
   - 趋势分析工具
   - 异常检测工具
   - 能耗分析工具

3. **决策建议系统**
   - LLM 生成决策建议
   - 发布 `LlmDecisionProposed` 事件
   - 用户确认或自动执行

4. **主动优化**
   - 自动发现优化机会
   - 自动创建优化规则
   - 主动通知用户

### 阶段 6: 前端重构 (Week 8-9)

**目标**: 前端显示真实数据和 LLM 决策

1. **实时数据**
   - WebSocket 订阅设备事件
   - 实时显示指标变化
   - 实时显示规则触发

2. **LLM 决策界面**
   - 显示 LLM 决策建议
   - 用户确认/拒绝界面
   - 决策历史查看

3. **控制界面**
   - 设备控制
   - 规则管理
   - 工作流监控

---

## 五、核心数据流

### 5.1 设备数据 → 规则 → 动作

```
数据流:

MQTT Device (发布 metric)
    ↓
MqttAdapter (接收)
    ↓
EventBus::publish(DeviceMetric { device_id: "sensor1", metric: "temperature", value: 75.0 })
    ↓
RuleEngine (订阅 DeviceMetric)
    ↓
评估: WHEN sensor1.temperature > 50
    ↓
EventBus::publish(RuleTriggered { rule_id: "alert_1" })
    ↓
EventHandler (订阅 RuleTriggered)
    ↓
执行动作: NOTIFY "温度过高"
```

### 5.2 LLM → 设备控制

```
控制流:

用户: "打开客厅的灯"
    ↓
LLM Agent (解析意图)
    ↓
Tool Call: control_device(device_id="light_1", command="turn_on")
    ↓
RealToolRegistry::execute_tool("control_device", args)
    ↓
MqttDeviceManager::send_command("light_1", "turn_on")
    ↓
MQTT Publish: cmnd/light_1/POWER "ON"
    ↓
设备执行 → EventBus::publish(DeviceCommandResult)
```

### 5.3 LLM 主动决策 (新增！)

```
自主决策流:

定时触发器 (每小时)
    ↓
EventBus::publish(PeriodicReviewTriggered)
    ↓
AutonomousAgent (订阅 PeriodicReviewTriggered)
    ↓
收集系统数据:
  - 设备状态
  - 规则执行历史
  - 告警统计
  - 能耗趋势
    ↓
LLM 分析数据
    ↓
生成决策建议:
  - "检测到设备 xyz 温度持续升高"
  - "建议: 创建规则 WHEN xyz.temperature > 80 DO turn_on_fan"
    ↓
EventBus::publish(LlmDecisionProposed {
  title: "预防性降温建议",
  actions: [create_rule, ...],
  confidence: 0.85
})
    ↓
前端显示决策建议
    ↓
用户确认 → 自动执行
    ↓
EventBus::publish(LlmDecisionExecuted)
```

### 5.4 用户与 LLM 交互

```
用户交互流:

用户: "帮我总结一下今天的设备状态"
    ↓
LLM Agent 调用工具:
  - list_devices() → 获取设备列表
  - query_data(..., "last_24h") → 获取24小时数据
  - analyze_trends(...) → 分析趋势
    ↓
LLM 生成总结:
  "今天有 15 个设备在线。
   温度传感器平均温度 25°C。
   检测到 2 个异常:
   - 设备 sensor3 在 14:00 温度异常升高
   - 设备 light5 在 10:00 离线"
    ↓
LLM 主动提出建议:
  "建议: 为 sensor3 创建高温告警规则"
    ↓
用户: "好的，创建"
    ↓
LLM 执行: create_rule(...)
```

---

## 六、关键文件结构

### 重构后的目录结构

```
crates/
├── core/                    # 核心类型和事件
│   ├── src/
│   │   ├── event.rs         # 统一事件类型
│   │   ├── eventbus.rs      # 事件总线
│   │   ├── message.rs       # 消息类型
│   │   └── lib.rs
│
├── devices/                 # 设备管理
│   ├── src/
│   │   ├── adapter.rs       # DeviceAdapter trait
│   │   ├── adapter_manager.rs  # 适配器管理器
│   │   ├── adapters/        # 适配器实现
│   │   │   ├── mod.rs
│   │   │   ├── mqtt.rs
│   │   │   ├── hass.rs      # HASS 只是其中一个
│   │   │   ├── modbus.rs
│   │   │   └── http.rs
│   │   ├── mdl/             # 设备类型定义
│   │   └── telemetry.rs     # 时序存储
│
├── rules/                   # 规则引擎
│   ├── src/
│   │   ├── engine.rs        # 规则引擎
│   │   ├── dsl.rs           # DSL 解析
│   │   └── integration.rs   # 事件集成
│
├── workflow/                # 工作流引擎
│   ├── src/
│   │   ├── workflow.rs      # 工作流定义
│   │   ├── executor.rs      # 执行器
│   │   └── triggers/        # 触发器
│   │       ├── event.rs     # 事件触发器
│   │       ├── cron.rs      # 定时触发器
│   │       ├── manual.rs    # 手动触发器
│   │       └── llm_decision.rs  # LLM 决策触发器
│
├── agent/                   # LLM Agent
│   ├── src/
│   │   ├── agent.rs         # Agent 核心
│   │   ├── autonomous.rs    # 自主决策 (新增!)
│   │   ├── tools/           # 工具集成
│   │   │   ├── registry.rs  # 工具注册表
│   │   │   └── real.rs      # 真实工具实现
│   │   └── session.rs       # 会话管理
│
├── api/                     # API Server
│   ├── src/
│   │   ├── handlers/        # HTTP/WebSocket handlers
│   │   ├── middleware/      # 中间件
│   │   └── server.rs        # 服务器
│
└── storage/                 # 存储
    ├── src/
    │   ├── telemetry.rs     # 时序数据 (redb)
    │   ├── state.rs         # 状态数据 (sled)
    │   ├── events.rs        # 事件日志
    │   └── decisions.rs     # LLM 决策历史 (新增!)
```

---

## 七、实施优先级

### P0 (Critical) - 事件驱动核心

1. **统一事件类型** - `crates/core/src/event.rs`
2. **事件总线** - `crates/core/src/eventbus.rs`
3. **设备适配器接口** - `crates/devices/src/adapter.rs`

### P0 (Critical) - LLM 工具集成

4. **真实工具实现** - `crates/agent/src/tools/real.rs`
5. **工具注册表** - `crates/agent/src/tools/registry.rs`

### P1 (High) - 数据流集成

6. **规则引擎事件集成** - `crates/rules/src/integration.rs`
7. **MQTT 适配器** - `crates/devices/src/adapters/mqtt.rs`
8. **适配器管理器** - `crates/devices/src/adapter_manager.rs`

### P1 (High) - LLM 自主能力 ⭐

9. **自主 Agent 框架** - `crates/agent/src/autonomous.rs`
10. **定期审查机制** - 定时触发 + 数据收集
11. **决策建议系统** - LlmDecisionProposed 事件 + 处理
12. **分析工具集** - 趋势分析、异常检测

### P2 (Medium) - 工作流和前端

13. **工作流事件触发器** - `crates/workflow/src/triggers/event.rs`
14. **前端实时事件** - WebSocket 订阅
15. **LLM 决策界面** - 前端组件

### P3 (Low) - 优化和扩展

16. **事件持久化**
17. **事件回放**
18. **性能监控**

---

## 八、成功标准

### 功能完整性
- [ ] 设备数据变化自动触发规则评估
- [ ] 规则可以控制设备
- [ ] LLM 可以查询设备和控制设备
- [ ] 工作流可以响应设备事件
- [ ] 前端实时显示设备状态和规则触发
- [ ] **LLM 可以定期分析系统状态**
- [ ] **LLM 可以主动提出优化建议**
- [ ] **LLM 决策可以自动执行**

### LLM 能力验证
- [ ] LLM 能回答"今天系统怎么样"
- [ ] LLM 能检测异常并告警
- [ ] LLM 能主动创建优化规则
- [ ] LLM 能解释规则触发原因
- [ ] LLM 能总结历史数据趋势

### 性能指标
- [ ] 事件延迟 < 10ms (从设备到规则评估)
- [ ] 支持并发 1000+ 设备
- [ ] 规则评估延迟 < 1ms
- [ ] LLM 响应时间 < 5s (复杂分析)

### 代码质量
- [ ] 单一职责: 每个文件 < 500 行
- [ ] 松耦合: 模块通过事件通信
- [ ] 可测试: 事件驱动易于单元测试
