...
import { AlertCircle, Plus, Check, RefreshCw, Trash2, Eye, Bell } from "lucide-react"
import { PageLayout } from "@/components/layout/PageLayout"
import { LoadingState, EmptyState, Pagination, AlertBadge, PageTabs, PageTabsContent, BulkActionBar } from "@/components/shared"
  ...
  return (
    <PageLayout>
      <PageTabs
        tabs={tabs}
        activeTab={filter}
        onTabChange={(v) => setFilter(v as AlertFilter)}
        actions={[
          { label: t('common:refresh'), icon: <RefreshCw className="h-4 w-4" />, onClick: fetchAlerts, variant: 'outline' },
          { label: t('alerts:createAlert'), icon: <Plus className="h-4 w-4" />, onClick: () => setCreateDialogOpen(true) },
        ]}
      >
